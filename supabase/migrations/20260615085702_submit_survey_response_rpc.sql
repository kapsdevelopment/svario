  ...
  loop
    ...
    if v_question.type = 'free_text' then
      if v_likert_value is not null or cardinality(v_option_ids) > 0 then
        raise exception 'Free-text questions cannot include choices or Likert values.';
      end if;
    ...
    elsif v_question.type = 'likert_1_5' then
      if v_free_text is not null or cardinality(v_option_ids) > 0 then
        raise exception 'Likert questions cannot include free text or choices.';
      end if;
      ...
      if v_likert_value < 1 or v_likert_value > 5 then
        raise exception 'Likert values must be between 1 and 5.';
      end if;
      ...
      if v_free_text is not null or v_likert_value is not null then
        raise exception 'Choice questions cannot include free text or Likert values.';
      end if;
